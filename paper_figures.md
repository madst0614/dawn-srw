...
```python
# evaluate.py - estimate_flops() [line 50-115]
...
| Fig 5 Factual neurons | shared pool, set dedup | ⚠️ Fix needed | substring → exact match 수정 필요, 단일 토큰 검증 추가 |
...
**코드 위치**: `scripts/evaluation/evaluate.py` line 50-115
...
### Action Items (남은 작업)
1. **Fig 5**: behavioral.py line 666 - substring match를 exact match로 수정, 단일 토큰 검증 추가
