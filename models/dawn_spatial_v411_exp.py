"""
DAWN-SRW v4.1.1: v4.1 two-stage gate + v4.0.6 emb handling

Same two-stage gate (activation × intensity), static tau, Σintensity
denominator as v4.1 — the change is narrow: emb is no longer
forward-normalised to unit. ||emb|| becomes a learnable DoF (v4.0.6
style) so per-neuron magnitude differences can drive competitive
sparsity again. read / write are still forward-normalised.
...
def make_sharded_srw(mesh, max_chunk_size=2048, dead_threshold=0.01,
    ...
    v4.1.1: emb is used as-is (no forward unit-norm). ||emb|| is a
    learnable DoF regulated by weight-decay, which restores the
    v4.0.6 competitive-sparsity regime where per-neuron magnitude
    differences drive gating. read/write still forward-normalise.
    """
    ...
    def fused_gate_srw(x, h, emb_local, tau_offset, read_local, write_local):
        ...
        # v4.1.1: emb used as-is (no forward unit-norm). ||emb|| is a
        # learnable DoF — weight-decay controls its size. Restores
        # v4.0.6 sparsity regime where per-neuron magnitude differences
        # drive competitive gating.
...
def make_sharded_srw_paired(mesh, max_chunk_size=2048, dead_threshold=0.01,
    ...
    v4.1.1: emb used as-is (see make_sharded_srw docstring).
    """
    _model_axis_size = mesh.shape['model']
    _data_axis_size = mesh.shape['data']
    _dead_thresh = jnp.float32(dead_threshold)
    ...
    def fused_gate_srw_paired(x, h, emb_local, tau_offset, read_local, write_local):
        ...
        h_bf = h.astype(jnp.bfloat16)
        x_bf = x.astype(jnp.bfloat16)
        # v4.1.1: emb as-is; norm is a learnable DoF (v4.0.6-style).
        emb_bf = emb_local.astype(jnp.bfloat16)
        read_bf = read_local.astype(jnp.bfloat16)
        write_bf = write_local.astype(jnp.bfloat16)
...
class NeuronPool(nn.Module):
    ...
    def setup(self):
        ...
        # Sense (routing, low-dim). Unit-norm init; v4.1.1 does NOT
        # re-normalise at forward time — ||emb|| is a learnable degree
        # of freedom (WD controls size). Restores v4.0.6 competitive
        # sparsity regime where per-neuron magnitude differences matter.
...
def _attn_forward(x, pool_params, router_params, expand_O_kernel, rng,
    ...
    # v4.1.1: emb used as-is (no forward normalisation). Names kept for
    # downstream readability but these are not unit vectors post-init.
    qk_emb_unit = qk_emb
    v_emb_unit = v_emb
...
def _know_forward(x, pool_params, router_params, rng,
    ...
    # v4.1.1: emb used as-is (no forward normalisation).
...
def _srw_inference(x, h, emb_norm, tau_offset, w_read, w_write):
    """Non-chunked SRW for inference. v4.1.1: gate = activation × intensity,
    den = max(Σintensity, 1.0). emb_norm is passed as-is (v4.0.6-style;
    caller does not normalise)."""
    scores = h @ emb_norm.T
...
def _attn_forward_cached(x, pool_params, router_params, expand_O_kernel,
    ...
    # v4.1.1: emb as-is (matches training path — no forward unit-norm).
...
def _know_forward_inference(x, pool_params, router_params):
    ...
    # v4.1.1: emb as-is (matches training path).
