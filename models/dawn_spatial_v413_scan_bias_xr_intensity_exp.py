"""
DAWN-SRW v4.1.3: scan-bias tau + XR intensity gate
...
Changelog:
  ...
  spatial-r1-v4.1.3 scan-bias + XR intensity wiring (2026-04-26):
    - spatial-r1-v4.1.3 trains from dawn_spatial_v413_scan_bias_xr_intensity_exp.py.
    - Keeps v4.1.2 bounded scan-bias tau:
        scan = scan_scale * tanh(scan_bias)
        tau = s_mean + tau_offset * s_std - scan / max(s_std, scan_std_floor)
    - Restores v4.1.1 gate form:
        selection = sigmoid(sharpness * ((score - tau) - threshold))
        intensity = x @ read.T
        output = sum(selection * intensity * write) / max(sum(abs(intensity)), 1)
    ...
    def fused_gate_srw(x, h, emb_local, tau_offset, scan_bias,
        ...
        if analysis:
            ...
            def gate_srw_step(carry, i):
                ...
                intensity_abs = jnp.abs(intensity.astype(jnp.float32))
                ...
                chunk_z_sum = intensity_abs.sum(axis=-1, keepdims=True)
    ...
    def fused_gate_srw_paired(x, h, emb_local, tau_offset, scan_bias,
        ...
        if analysis:
            ...
            def gate_srw_step(carry, i):
                ...
                chunk_z_sum = intensity_abs[:, :, None, :].sum(axis=-1, keepdims=True)
                chunk_z_lt_075 = ((selection > 0.05) & (selection < 0.95)
                                  ).astype(jnp.float32).sum(axis=-1, keepdims=True)
                chunk_z_lt_030 = ((selection > 0.2) & (selection < 0.8)
                                  ).astype(jnp.float32).sum(axis=-1, keepdims=True)
...
def _srw_inference(x, h, emb_norm, tau_offset, scan_bias, w_read, w_write):
...
def _srw_inference_with_gates(x, h, emb_norm, tau_offset, scan_bias, w_read, w_write):
...
def build_suppressed_forward(params, model_cfg, suppress_masks):
    ...
    def _srw_sup(x, h, emb_n, tau_off, scan_bias, w_read, w_write, mult):
    ...
    def forward_fn(input_ids):
        ...
        for i in range(n_layers):
            ...
            Q = _srw_sup(normed, h_Q, qk_n, tau_all[:,:,0:1], scan_bias_all[:,:,0:1], pp['qk_read'], pp['qk_write'], qk_mult)
            K = _srw_sup(normed, h_K, qk_n, tau_all[:,:,1:2], scan_bias_all[:,:,1:2], pp['qk_read'], pp['qk_write'], qk_mult)
            V = _srw_sup(normed, h_V, v_n, tau_all[:,:,2:3], scan_bias_all[:,:,2:3], pp['v_read'], pp['v_write'], v_mult)
            _qk_s = pp['qk_scale']
            _v_s = pp['v_scale']
            Q = Q * _qk_s
            K = K * _qk_s
            ...
            x = x + _srw_sup(normed, h_k, kn_n, tau_k, scan_bias_k, pp['know_read'], pp['know_write'], know_mult) * pp['know_scale']
