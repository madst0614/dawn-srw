"""
DAWN-SRW v4.1.5.8: SRW neurons and Residual State Transition.

This file contains the v4.1.5.8 implementation. DAWN neurons are
...
write vector writes that scalar back to the residual stream:
    ...
    c_i = gate_i * <x, r_i> * w_i
    out = sum_i c_i / max(sum_i gate_i * |<x, r_i>| * ||w_i||, 1.0)
...
For each token and pool:
    ...
    c_i = gate_i * <x, r_i> * w_i
    contrib_den = sum_i gate_i * |<x, r_i>| * ||w_i||
    out = sum_i c_i / max(contrib_den, rw_contrib_den_floor)
...
* The SRW denominator is the RW contribution magnitude:
  sum(gate * abs(read_i(x)) * ||write_i||).  The old sum(gate) denominator is
  retained only as a diagnostic.
* RWCompose produces a bounded transition direction/coherence vector.  The
  learnable pool output scales control branch step size.
...
#   intensity     = EPSILON + min(active_margin, MAX_INTENSITY)
#   gate          = activation * intensity
#   den           = max(sum(gate * abs(<x, r_i>) * ||w_i||), 1.0)
# ================================================================
...
DEFAULT_D_ROUTE = 64
RW_CONTRIB_DEN_FLOOR = 1.0     # main SRW denominator floor; keeps low-den gradients bounded
...
def make_sharded_srw(mesh, max_chunk_size=2048, dead_threshold=0.01,
                     ...
                     rw_contrib_den_floor=RW_CONTRIB_DEN_FLOOR):
    ...
    v4.1 gate:
        ...
        intensity     = epsilon + min(active_margin, max_intensity)
        gate          = activation * intensity
        contrib_den   = sum(gate * abs(<x, read>) * ||write||)
        den           = max(contrib_den, rw_contrib_den_floor)
    ...
    _rw_den_floor = jnp.float32(rw_contrib_den_floor)
    ...
    _slim_out_specs = (
        ...
        P(),                     # gate_den_sum_mean scalar (old sum(gate))
        P(),                     # activation_cost_mean scalar
        P(),                     # current_cost_mean scalar
        P(),                     # contrib_den_sum_mean scalar
        P(),                     # contrib_den_max scalar
    ...
    def fused_gate_srw(x, h, emb_local, tau_offset, raw_scan_offset,
        ...
        def route_chunk(start):
            ...
            # v4.1.5.8: use raw read/write vectors.
        ...
        if analysis:
            ...
            def gate_srw_step(carry, i):
                (out, total_weighted_cost, total_gate_sq, total_gate_max, total_active,
                 total_strong, total_phi_binary, total_den_cost, total_contrib_den,
                ...
                wc_f32 = wc.astype(jnp.float32)
                write_norm = jnp.sqrt(
                    jnp.sum(jnp.square(wc_f32), axis=-1) + jnp.float32(1e-8))
                a = gate * xr_f
                c_out = (a.astype(jnp.bfloat16) @ wc).astype(jnp.float32)
                chunk_weighted = gate.sum(axis=-1, keepdims=True)
                chunk_contrib_den = (
                    gate * jnp.abs(xr_f) * write_norm[None, None, :]
                ...
                return (out + c_out,
                        ...
                        total_contrib_den + chunk_contrib_den,
            ...
            (raw_out, total_weighted_cost, total_gate_sq, total_gate_max, total_active, total_strong,
             total_phi_binary, total_den_cost, total_contrib_den, total_activation_cost,
             ...
             total_int_cap_count, diag_vals), _ = jax.lax.scan(
                ...
                (jnp.zeros((B, S, D), dtype=jnp.float32),
                 z1, z1, jnp.full((B, S, 1), -1e9), z1, z1, z1, z1, z1, z1, z1, z1, z1, z1,
            ...
            def gate_srw_step(carry, i):
                (out, total_weighted_cost, total_gate_sq, total_gate_max, total_active,
                 total_strong, total_den_cost, total_contrib_den,
                 total_activation_cost, total_current_cost,
                 total_dead_penalty, total_dead_count,
                 total_int_max, total_full_gate, total_kept_count,
                 total_int_cap_count, diag_vals) = carry
                ...
                chunk_active = (activation > 0.5).astype(jnp.float32).sum(axis=-1, keepdims=True)
                chunk_strong = (activation > 0.9).astype(jnp.float32).sum(axis=-1, keepdims=True)
                # sum(gate) is retained as the backward-compatible gate-den diagnostic.
                max_gate_chunk = gate.max(axis=(0, 1))
                mean_score_chunk = scores_f.mean(axis=(0, 1))
                max_gate_chunk = jax.lax.pmax(
                    jax.lax.stop_gradient(max_gate_chunk), 'data')
            ...
            (raw_out, total_weighted_cost, total_gate_sq, total_gate_max, total_active, total_strong,
             total_den_cost, total_contrib_den, total_activation_cost, total_current_cost,
             total_dead_penalty, total_dead_count,
             total_int_max, total_full_gate, total_kept_count,
             total_int_cap_count, diag_vals), _ = jax.lax.scan(
                gate_srw_step,
                (jnp.zeros((B, S, D), dtype=jnp.float32),
                 z1, z1, jnp.full((B, S, 1), -1e9), z1, z1, z1, z1, z1, z1,
        ...
        # v4.1.5.8: actual normalization uses RW contribution magnitude.
        # The old gate denominator is kept below only as a diagnostic.
        global_den_cost = jax.lax.psum(total_den_cost, 'model')
        global_contrib_den = jax.lax.psum(total_contrib_den, 'model')
        ...
        den = jnp.maximum(global_contrib_den, _rw_den_floor)
        out = raw_out / den
        ...
        # Action path above keeps global_contrib_den live for
        ...
        global_contrib_den_m = jax.lax.stop_gradient(global_contrib_den)
        ...
        den_cost_mean = global_weighted_cost_m.mean()
        ...
        if False and _local_diagnostics:
            ...
            def _local_diag_step(carry, i):
                ...
                out_den = jnp.maximum(
                    global_contrib_den_m[:, :, None, :], _rw_den_floor)
    ...
    def fused_gate_srw_paired(x, h, emb_local, tau_offset, raw_scan_offset,
            ...
            (raw_out, total_weighted_cost, total_gate_sq, total_gate_max, total_active, total_strong,
             ...
             total_int_cap_count, diag_vals), _ = jax.lax.scan(
                ...
                (jnp.zeros((B, S, 2, D), dtype=jnp.float32),
                 ...
                 z1_r, z1_r, z1_r, z1_r, z1_r, z1_r, z1_r, z1_r, z1_r, z1_r,
                 jnp.float32(0.0), jnp.float32(0.0),
        ...
        if False and _local_diagnostics:
            ...
            def _local_diag_step(carry, i):
                ...
                out_den = jnp.maximum(global_contrib_den_m, _rw_den_floor)
                top1_share = gate / gate_den
...
class NeuronPool(nn.Module):
    ...
    def setup(self):
        ...
        # v4.1.5.8 uses raw read/write vector magnitudes.
...
class DAWN(nn.Module):
    """DAWN-SRW v4.1.5.8 with Attention Layers and RST Layers."""
    __version__ = "spatial-r1-v4.1.5.8"
    ...
    # Optional safety bound on router-produced tau offsets. This is a
    # forward-time clamp, not a new parameter, so old checkpoints remain
    # compatible. Set to None/0 to disable clipping.
    ...
    def __call__(self, input_ids, labels=None, attention_mask=None,
        ...
        if self.is_initializing():
            ...
            def scan_body(carry, xs):
                ...
                slim_ys = (attn_aux, rst_aux,
                           ...
                           a_contrib_den_sum, k_contrib_den_sum,
                           a_contrib_den_max, k_contrib_den_max,
            ...
            attn_contrib_den_sum_all, rst_contrib_den_sum_all,
            attn_contrib_den_max_all, rst_contrib_den_max_all,
        ...
        result = {
            ...
            # Denominator diagnostics: old gate sum plus v4.1.5.8 RW contribution denominator.
            ...
            'rst_compose_norm_max': rst_compose_norm_max_all.max(),
            'attn_coherence_max': attn_compose_norm_max_all.max(),
...
def _srw_chunked(x, h, emb, tau_offset, w_read, w_write, n_chunks,
                 raw_scan_offset=None):
    """Non-sharded/fallback chunked SRW with v4.1.5.8 denominator semantics."""
    B, S, D = x.shape
    N = emb.shape[0]
    cs = N // n_chunks
    ...
    z1 = jnp.zeros((B, S, 1), dtype=jnp.float32)

    if raw_scan_offset is None:
        raw_scan_offset = jnp.zeros_like(tau_offset)
    ...
    def stats_step(carry, i):
        ...
        s_sum = s_sum + scores_f.sum(axis=-1, keepdims=True)
        sq_sum = sq_sum + (scores_f ** 2).sum(axis=-1, keepdims=True)
        per_neuron_score = scores_f.mean(axis=(0, 1))
        ns_sum = ns_sum + per_neuron_score.sum()
        ns_sq = ns_sq + (per_neuron_score ** 2).sum()
        return (s_sum, sq_sum, ns_sum, ns_sq), None
    ...
    (s_sum, sq_sum, ns_sum, ns_sq), _ = jax.lax.scan(
        stats_step, (z1, z1, z_scalar, z_scalar), jnp.arange(n_chunks))
    ...
    @jax.checkpoint
    def gate_srw_step(carry, i):
        (out, total_gate, total_contrib_den, total_gate_sq, total_gate_max,
         total_active, total_strong, total_g_log_g, total_int_max) = carry
        s = i * cs
        ec = jax.lax.dynamic_slice_in_dim(emb_bf, s, cs, axis=0)
        ...
        a = gate * xr
        c_out = (a.astype(jnp.bfloat16) @ wc).astype(jnp.float32)
        chunk_gate = gate.sum(axis=-1, keepdims=True)
        chunk_contrib_den = (
            gate * jnp.abs(xr) * write_norm[None, None, :]
        ).sum(axis=-1, keepdims=True)
        g_safe = gate + 1e-8
        return (out + c_out,
                total_gate + chunk_gate,
                total_contrib_den + chunk_contrib_den,
                total_gate_sq + jnp.square(gate).sum(axis=-1, keepdims=True),
                jnp.maximum(total_gate_max, gate.max(axis=-1, keepdims=True)),
                total_active + (activation > 0.5).astype(jnp.float32).sum(axis=-1, keepdims=True),
                total_strong + (activation > 0.9).astype(jnp.float32).sum(axis=-1, keepdims=True),
                total_g_log_g + (gate * jnp.log(g_safe)).sum(axis=-1, keepdims=True),
                jnp.maximum(total_int_max, intensity.max())), None

    (raw_out, gate_sum, contrib_den, gate_sq, gate_max, active, strong,
     g_log_g, int_max), _ = jax.lax.scan(
        gate_srw_step,
        (jnp.zeros((B, S, D), dtype=jnp.float32),
         z1, z1, z1, jnp.full((B, S, 1), -1e9),
         z1, z1, z1, jnp.float32(0.0)),
        jnp.arange(n_chunks))

    den = jnp.maximum(contrib_den, jnp.float32(RW_CONTRIB_DEN_FLOOR))
    out = (raw_out / den).astype(jnp.float32)
    gate_sum_m = jax.lax.stop_gradient(gate_sum)
    contrib_den_m = jax.lax.stop_gradient(contrib_den)
    compose_norm = jnp.linalg.norm(jax.lax.stop_gradient(out), axis=-1)
    gate_eff_n = (gate_sum_m ** 2) / (jax.lax.stop_gradient(gate_sq) + 1e-8)
    top1_gate_frac = jax.lax.stop_gradient(gate_max) / jnp.maximum(gate_sum_m, 1e-8)
    return (
        out, active / N, gate_max, score_lb, s_std.mean(),
        gate_sum_m.mean(), active.mean(), strong / N,
        gate_sum_m / (active + 1e-8), jax.lax.stop_gradient(tau).mean(),
        jnp.float32(0.0), jnp.float32(0.0), int_max,
        gate_sum_m.mean(), jnp.float32(0.0), jnp.float32(0.0),
        contrib_den_m.mean(), contrib_den_m.max(),
        compose_norm.mean(), compose_norm.max(),
        (contrib_den_m / jnp.maximum(gate_sum_m, 1e-8)).mean(),
        gate_eff_n.mean(),
        (gate_eff_n / jnp.maximum(active, 1.0)).mean(),
        top1_gate_frac.mean(), top1_gate_frac.max(),
    )
...
def _srw_inference(x, h, emb, tau_offset, raw_scan_offset, w_read, w_write):
    ...
    # v4.1.5.8: raw read/write vectors with RW-contribution normalization.
    ...
    a = gate * xr
    raw_out = a @ w_n
    write_norm = jnp.sqrt(jnp.sum(jnp.square(w_n), axis=-1) + jnp.float32(1e-8))
    contrib_den = (
        gate * jnp.abs(xr) * write_norm[None, None, :]
    ).sum(axis=-1, keepdims=True)
    den = jnp.maximum(contrib_den, jnp.float32(RW_CONTRIB_DEN_FLOOR))
