"""
DAWN-SRW v4.1.1: Tau-bias selection * intensity * write
...
**Stage 2 — Intensity (read projection)**:
    intensity = x · read

  Embedding scores only decide which neurons participate. Magnitude is
  carried by the signed read projection, then emitted along write:
    out = sum(selection × intensity × write) / max(sum(selection × |intensity|), 1)

### Denominator

  selection-count based.  The denominator is no longer driven by
  score-margin intensity, so embedding scores stay focused on selection
  while read/write learn contribution magnitude and direction.

### Static tau (no dynamic shift)
    tau = s_mean + tau_offset × s_std + tau_bias

  tau_offset and tau_bias are learnable per-token via Dense(x).  v4.0.6's dynamic
...
3. Dynamic tau removed; relative tau remains, now with additive learnable tau_bias.
...
Changelog:
  spatial-r1-v4.1.1 (2026-04-25):
    - Tau-bias selection * intensity * write experiment.
    - tau = s_mean + tau_offset * s_std + tau_bias.
    - score/emb path produces selection only:
        selection = sigmoid(sharpness * ((score - tau) - threshold))
    - read path carries signed intensity:
        out = sum(selection * (x @ read.T) * write)
              / max(sum(selection * abs(x @ read.T)), 1)
  ...
  spatial-r1-v4.1 (2026-04-21; emb forward-norm reverted 2026-04-23;
    ...
    - Relative tau-bias added: tau = s_mean + tau_offset * s_std + tau_bias.
      tau_bias is zero-init and gives wx+b-style fine adjustment without
      reintroducing v4.0.6 inverse-std dynamic shift.
  ...
  spatial-r1-v4.1 tau-bias wiring (2026-04-25):
    - spatial-r1-v4.1 trains from dawn_spatial_v41_tau_bias_exp.py.
    - tau_bias has no extra L2 term; it only follows the existing AdamW
  ...
  spatial-r1-v4.0.6 (2026-04-20):
    - Gate confidence: Φ(z) (normal CDF) → sigmoid(scores - tau).
      gate = where(z > 0, z · sigmoid(scores - tau), 0).
    ...
    - Dead-only penalty: for each pool, identify dead neurons (max gate
      across the step-batch below 1e-4) and add
          sum_j max(-mean_score_j, 0) · 1[dead_j]
...
ACTIVATION_CUTOFF = 0.01       # kept for config compatibility/log labels
EPSILON = 1e-4                 # kept for config compatibility
MAX_INTENSITY = 10.0           # kept for config compatibility
...
def make_sharded_srw(mesh, max_chunk_size=2048, dead_threshold=0.01,
    ...
    `analysis=True`: returns the SLIM tuple followed by 9 extra
    observational scalars/arrays (score_skew, score_kurt, apt_std,
    gate_entropy, phi_binary, z_lt_075_frac, z_lt_030_frac, z_sum_out,
    int_cap_frac). Used by analysis_step at val time only.
    """
    ...
    _slim_out_specs = (
        ...
        P(),                     # lb_loss scalar
        P(),                     # score_std scalar
        P(),                     # gate_sum scalar (Σgate, observational)
    ...
    _analysis_extra_specs = (
        ...
        P(),                     # z_sum_out (Σintensity, observational)
        P(),                     # score_kurt scalar
    ...
    @partial(shard_map, mesh=mesh,
             in_specs=(P('data', None, None),    # x [B,S,D]
                       ...
                       P('data', None, None),    # tau_bias [B,S,1]
                       P('model', None),          # read [N_local, D]
                       P('model', None)),         # write [N_local, D]
             out_specs=_out_specs,
             check_rep=False)
    def fused_gate_srw(x, h, emb_local, tau_offset, tau_bias, read_local, write_local):
        ...
        # v4.1+bias: relative tau with wx+b-style fine adjustment.
        tau = s_mean + tau_offset * s_std + tau_bias
        ...
        if analysis:
            ...
            # Kurtosis via E[(X-μ)^4] = E[X^4] - 4μE[X^3] + 6μ²σ² + 3μ⁴
        ...
        if analysis:
            ...
            def gate_srw_step(carry, i):
                ...
                chunk_int_max = jnp.abs(selection_bf * intensity).max()
                ...
                chunk_z_sum = jnp.abs(intensity.astype(jnp.float32)
                                      ).sum(axis=-1, keepdims=True)
                chunk_z_lt_075 = ((selection > 0.05) & (selection < 0.95)
                                  ).astype(jnp.float32).sum(axis=-1, keepdims=True)
                chunk_z_lt_030 = ((selection > 0.2) & (selection < 0.8)
                                  ).astype(jnp.float32).sum(axis=-1, keepdims=True)
                g_safe = selection + 1e-8
                chunk_g_log_g = (selection * jnp.log(g_safe)).sum(axis=-1, keepdims=True)
                ...
                return (out + c_out,
                        ...
                        jnp.maximum(total_gate_max, selection.max(axis=-1, keepdims=True)),
            ...
            (raw_out, total_weighted_cost, total_gate_max, total_active, total_strong,
             total_z_sum, total_dead_penalty, total_dead_count,
             total_int_max), _ = jax.lax.scan(
                gate_srw_step,
                (jnp.zeros((B, S, D), dtype=jnp.float32),
                 z1, jnp.full((B, S, 1), -1e9), z1, z1, z1,
                 jnp.float32(0.0), jnp.float32(0.0), jnp.float32(0.0)),
                jnp.arange(nc))

        global_weighted_cost = jax.lax.psum(total_weighted_cost, 'model')  # Σgate
        ...
        return slim_out + (phi_binary_frac, z_lt_075_frac, z_lt_030_frac,
                           ...
                           z_sum_out, score_kurt, int_cap_frac_out)
...
def make_sharded_srw_paired(mesh, max_chunk_size=2048, dead_threshold=0.01,
    ...
    tau_offset and tau_bias are [B,S,2,1].
    ...
    @partial(shard_map, mesh=mesh,
             in_specs=(P('data', None, None),        # x [B,S,D]
                       ...
                       P('data', None, None, None),  # tau_bias [B,S,2,1]
                       P('model', None),              # read [N_local, D]
                       P('model', None)),             # write [N_local, D]
             out_specs=_out_specs,
             check_rep=False)
    def fused_gate_srw_paired(x, h, emb_local, tau_offset, tau_bias, read_local, write_local):
        N_local = emb_local.shape[0]
        nc = max(1, N_local // max_chunk_size)
        while N_local % nc != 0 and nc < N_local:
            nc += 1
        ...
        B, S, D = x.shape
        # h: [B,S,2,d_route], tau_offset/tau_bias: [B,S,2,1]
        h_bf = h.astype(jnp.bfloat16)
        x_bf = x.astype(jnp.bfloat16)
        ...
        s_std = jnp.sqrt(global_sq / N_total - s_mean ** 2) + 1e-8
        # v4.1+bias: relative tau with wx+b-style fine adjustment.
        tau = s_mean + tau_offset * s_std + tau_bias   # [B,S,2,1]
        ...
        if analysis:
            ...
            def gate_srw_step(carry, i):
                ...
                c_out = jnp.einsum('bsrn,nd->bsrd', selection_bf * intensity[:, :, None, :], wc).astype(jnp.float32)
                chunk_int_max = jnp.abs(selection_bf * intensity[:, :, None, :]).max()
                chunk_int_cap_count = jnp.float32(0.0)
                chunk_weighted = selection.sum(axis=-1, keepdims=True)           # [B,S,2,1]
                chunk_active = (selection > 0.5).astype(jnp.float32).sum(axis=-1, keepdims=True)
                chunk_strong = (selection > 0.9).astype(jnp.float32).sum(axis=-1, keepdims=True)
                chunk_phi_binary = ((selection > 0.1) & (selection < 0.9)
                                    ).astype(jnp.float32).sum(axis=-1, keepdims=True)
                chunk_z_sum = jnp.abs(intensity.astype(jnp.float32)
                                      )[:, :, None, :].sum(axis=-1, keepdims=True)
                chunk_z_lt_075 = ((selection > 0.05) & (selection < 0.95)
                                  ).astype(jnp.float32).sum(axis=-1, keepdims=True)
                chunk_z_lt_030 = ((selection > 0.2) & (selection < 0.8)
                                  ).astype(jnp.float32).sum(axis=-1, keepdims=True)
...
class Router(nn.Module):
    ...
    def setup(self):
        ...
        self.tau_know = nn.Dense(1, name='tau_know',
            kernel_init=nn.initializers.zeros,
            bias_init=lambda k, s, d: jnp.full(s, -0.5))
        # Additive tau fine-adjuster. Zero-init preserves old behavior at step 0.
        self.tau_bias_attn = nn.Dense(3, name='tau_bias_attn',
            kernel_init=nn.initializers.zeros,
            bias_init=nn.initializers.zeros)
        self.tau_bias_know = nn.Dense(1, name='tau_bias_know',
            kernel_init=nn.initializers.zeros,
            bias_init=nn.initializers.zeros)
...
def _attn_forward(x, pool_params, router_params, expand_O_kernel, rng,
    ...
    tau_bias_all = x @ router_params['tau_bias_attn']['kernel'] + router_params['tau_bias_attn']['bias']
    if analysis:
    ...
    tau_bias_QK = jnp.stack([tau_bias_all[:, :, 0:1], tau_bias_all[:, :, 1:2]], axis=2)
    qk_ret = fused_paired(x, h_QK, qk_emb_unit, tau_QK, tau_bias_QK, qk_read, qk_write)
    ...
    if analysis:
        (qk_phi_bin, qk_z075, qk_z030, qk_skew, qk_apt_std, qk_entropy,
         qk_z_sum, qk_kurt, qk_int_cap) = qk_ret[13:]
        qk_raw_norm = jnp.linalg.norm(QK_out, axis=-1).mean()
    Q = QK_out[:, :, 0, :] * qk_scale
    K = QK_out[:, :, 1, :] * qk_scale
    v_ret = fused_single(x, h_V, v_emb_unit, tau_all[:, :, 2:3], tau_bias_all[:, :, 2:3], v_read, v_write)
    ...
    if analysis:
        (v_phi_bin, v_z075, v_z030, v_skew, v_apt_std, v_entropy,
         v_z_sum, v_kurt, v_int_cap) = v_ret[13:]
...
def _know_forward(x, pool_params, router_params, rng,
    ...
    tau_bias = x @ router_params['tau_bias_know']['kernel'] + router_params['tau_bias_know']['bias']
    if analysis:
    ...
    know_ret = fused_single(x, h, know_emb_unit, tau, tau_bias, know_read, know_write)
    ...
    if analysis:
        (phi_binary_frac, know_z_lt_075_frac, know_z_lt_030_frac,
         ...
         know_z_sum, know_score_kurt, know_int_cap_frac) = know_ret[13:]
...
class DAWN(nn.Module):
    ...
    def __call__(self, input_ids, labels=None, attention_mask=None,
        ...
        if self.is_initializing():
            ...
            _ = self.router.tau_bias_attn(x)
            _ = self.router.tau_bias_know(x)
...
def _srw_inference(x, h, emb_norm, tau_offset, tau_bias, w_read, w_write):
    ...
    raw = scores_f32 - tau
    margin = raw - ACTIVATION_THRESHOLD
    selection = jax.nn.sigmoid(SHARPNESS * margin)
    ...
    den = jnp.maximum(jnp.abs(intensity).sum(axis=-1, keepdims=True), 1.0)
    out = raw_out.astype(jnp.float32) / den
    return out.astype(jnp.float32)


def _srw_inference_with_gates(x, h, emb_norm, tau_offset, tau_bias, w_read, w_write):
    ...
    margin = raw - ACTIVATION_THRESHOLD
    selection = jax.nn.sigmoid(SHARPNESS * margin)
    gate_norm = selection / jnp.maximum(selection.sum(axis=-1, keepdims=True), 1e-8)
...
def _attn_forward_cached(x, pool_params, router_params, expand_O_kernel,
    ...
    Q = _srw_inference(x, h_Q, qk_norm, tau_all[:, :, 0:1], tau_bias_all[:, :, 0:1],
                       pool_params['qk_read'], pool_params['qk_write'])
    K_new = _srw_inference(x, h_K, qk_norm, tau_all[:, :, 1:2], tau_bias_all[:, :, 1:2],
                           pool_params['qk_read'], pool_params['qk_write'])
    V_new = _srw_inference(x, h_V, v_norm, tau_all[:, :, 2:3], tau_bias_all[:, :, 2:3],
...
def _know_forward_inference(x, pool_params, router_params):
    ...
    tau = x @ router_params['tau_know']['kernel'] + router_params['tau_know']['bias']
    tau_bias = x @ router_params['tau_bias_know']['kernel'] + router_params['tau_bias_know']['bias']
    out = _srw_inference(x, h, know_norm, tau, tau_bias,
...
def prefill(params, model_cfg, input_ids):
    ...
    def prefill_layer(carry, xs):
        ...
        tau_bias_all = normed @ router_params['tau_bias_attn']['kernel'] + router_params['tau_bias_attn']['bias']

        Q = _srw_inference(normed, h_Q, qk_norm, tau_all[:, :, 0:1], tau_bias_all[:, :, 0:1],
                           pool_params['qk_read'], pool_params['qk_write'])
        K_val = _srw_inference(normed, h_K, qk_norm, tau_all[:, :, 1:2], tau_bias_all[:, :, 1:2],
                               pool_params['qk_read'], pool_params['qk_write'])
        V_val = _srw_inference(normed, h_V, v_norm, tau_all[:, :, 2:3], tau_bias_all[:, :, 2:3],
                               pool_params['v_read'], pool_params['v_write'])
        _qk_s = pool_params['qk_scale']
        _v_s = pool_params['v_scale']
        Q = Q * _qk_s
...
def vectorized_eval(params, model_cfg, all_tokens, batch_size=32):
    ...
    def forward_batch(input_ids):
        ...
        def layer_fn(x, bp):
            ...
            Q = _srw_inference(normed, h_Q, qk_norm, tau_all[:, :, 0:1], tau_bias_all[:, :, 0:1],
                               pool_params['qk_read'], pool_params['qk_write'])
            K = _srw_inference(normed, h_K, qk_norm, tau_all[:, :, 1:2], tau_bias_all[:, :, 1:2],
                               pool_params['qk_read'], pool_params['qk_write'])
            V = _srw_inference(normed, h_V, v_norm, tau_all[:, :, 2:3], tau_bias_all[:, :, 2:3],
                               pool_params['v_read'], pool_params['v_write'])
            _qk_s = pool_params['qk_scale']
            _v_s = pool_params['v_scale']
            Q = Q * _qk_s
            ...
            tau_bias_k = normed @ router_params['tau_bias_know']['kernel'] + router_params['tau_bias_know']['bias']
            know_out = _srw_inference(normed, h_k, know_norm, tau_k, tau_bias_k,
                                     pool_params['know_read'], pool_params['know_write'])
...
def analysis_forward(params, model_cfg, input_ids, mode='full'):
    ...
    def analysis_layer(carry, xs):
        ...
        Q, gate_Q_raw, gate_Q = _srw_inference_with_gates(
            normed, h_Q, qk_norm, tau_all[:, :, 0:1], tau_bias_all[:, :, 0:1],
            pool_params['qk_read'], pool_params['qk_write'])
        K, gate_K_raw, gate_K = _srw_inference_with_gates(
            normed, h_K, qk_norm, tau_all[:, :, 1:2], tau_bias_all[:, :, 1:2],
            pool_params['qk_read'], pool_params['qk_write'])
        V, gate_V_raw, gate_V = _srw_inference_with_gates(
            normed, h_V, v_norm, tau_all[:, :, 2:3], tau_bias_all[:, :, 2:3],
            pool_params['v_read'], pool_params['v_write'])
        _qk_s = pool_params['qk_scale']
        _v_s = pool_params['v_scale']
        Q = Q * _qk_s
        ...
        know_out, gate_Know_raw, gate_Know = _srw_inference_with_gates(
            normed, h_k, know_norm_w, tau_k, tau_bias_k,
            pool_params['know_read'], pool_params['know_write'])
...
def build_suppressed_forward(params, model_cfg, suppress_masks):
    ...
    def _srw_sup(x, h, emb_n, tau_off, tau_bias, w_read, w_write, mult):
        """SRW with optional gate suppression. v4.0.3: denominator = Σz^+ (floor 1.0)."""
        scores = h @ emb_n.T
        ...
        tau = s_mean + tau_off * s_std + tau_bias
        raw = sf - tau
        margin = raw - ACTIVATION_THRESHOLD
        selection = jax.nn.sigmoid(SHARPNESS * margin)
        if mult is not None:
            selection = selection * mult[None, None, :]
        # v4.1 free-norm: read / write used as-is.
        r_n = w_read
        w_n = w_write
        intensity = x @ r_n.T
    ...
    def forward_fn(input_ids):
        ...
        for i in range(n_layers):
            ...
            tau_bias_all = normed @ rp['tau_bias_attn']['kernel'] + rp['tau_bias_attn']['bias']

            Q = _srw_sup(normed, h_Q, qk_n, tau_all[:,:,0:1], tau_bias_all[:,:,0:1], pp['qk_read'], pp['qk_write'], qk_mult)
            K = _srw_sup(normed, h_K, qk_n, tau_all[:,:,1:2], tau_bias_all[:,:,1:2], pp['qk_read'], pp['qk_write'], qk_mult)
            V = _srw_sup(normed, h_V, v_n, tau_all[:,:,2:3], tau_bias_all[:,:,2:3], pp['v_read'], pp['v_write'], v_mult)
            _qk_s = pp['qk_scale']
            _v_s = pp['v_scale']
            Q = Q * _qk_s
            K = K * _qk_s
            ...
            tau_bias_k = normed @ rp['tau_bias_know']['kernel'] + rp['tau_bias_know']['bias']
            x = x + _srw_sup(normed, h_k, kn_n, tau_k, tau_bias_k, pp['know_read'], pp['know_write'], know_mult) * pp['know_scale']
