"""
DAWN-SRW v4.1.5.7: Directional Select with state-dependent selection sharpness.

This file contains the v4.1.5.7 implementation. DAWN neurons are
...
def _bounded_state_sharpness_range(raw, range_min=1.25, range_max=4.0):
    """Return log multiplicative range c for state_sharpness.

    state_sharpness = exp(c * tanh(raw_state_sharpness)).
    If c=log(2), state_sharpness lives in [0.5, 2.0].  We bound c itself
    in log-space so the token/state-level sharpness knob cannot run away.
    """
    log_min = jnp.log(jnp.float32(range_min))
    log_max = jnp.log(jnp.float32(range_max))
    return log_min + (log_max - log_min) * jax.nn.sigmoid(raw)


def _state_sharpness_from_raw(raw, sharpness_range):
    return jnp.exp(sharpness_range * jnp.tanh(raw))
...
def make_sharded_srw(mesh, max_chunk_size=2048, dead_threshold=0.01,
    ...
    _use_direction_routing = bool(use_direction_routing)
    _use_route_intensity = bool(use_route_intensity)
    ...
    def fused_gate_srw(x, h, emb_local, tau_offset, raw_scan_offset,
        ...
        h_route = _l2_normalize(h_f) if _use_direction_routing else h_f
        h_bf = h_route.astype(jnp.bfloat16)
        state_sharpness_f = state_sharpness.astype(jnp.float32)
        ...
        def route_chunk(start):
            ...
            ec_route = _l2_normalize(ec_f) if _use_direction_routing else ec_f
            rc_f = rc.astype(jnp.float32)
            wc_f = wc.astype(jnp.float32)
            # v4.1.5.7: routing uses normalized route embeddings; read/write stay raw.
            return ec_route.astype(jnp.bfloat16), rc_f.astype(jnp.bfloat16), wc_f.astype(jnp.bfloat16)
        ...
        if analysis:
            ...
            def gate_srw_step(carry, i):
                ...
                if _use_route_intensity:
                    ...
                    intensity = jnp.ones_like(activation)
                    base_gate = activation
                if _prune_enabled:
...
def make_sharded_srw_paired(mesh, max_chunk_size=2048, dead_threshold=0.01,
    ...
    @partial(shard_map, mesh=mesh,
             in_specs=(P('data', None, None),        # x [B,S,D]
                       ...
                       P('data', None, None, None),  # state_sharpness [B,S,2,1]
                       P('model', None),              # read [N_local, D]
                       P('model', None)),             # write [N_local, D]
             out_specs=_out_specs,
             check_rep=False)
    def fused_gate_srw_paired(x, h, emb_local, tau_offset, raw_scan_offset,
        ...
        B, S, D = x.shape
        # h: [B,S,2,d_route], tau_offset/raw_scan_offset/state_sharpness: [B,S,2,1]
        h_f = h.astype(jnp.float32)
        h_route = _l2_normalize(h_f) if _use_direction_routing else h_f
        h_bf = h_route.astype(jnp.bfloat16)
        state_sharpness_f = state_sharpness.astype(jnp.float32)
...
class Router(nn.Module):
    ...
    state_sharpness_range_min: float = 1.25
    state_sharpness_range_max: float = 4.0
    ...
    def setup(self):
        ...
        # v4.1.5.7: state-dependent selection sharpness.  raw heads are
        # zero-initialized so training starts at state_sharpness = 1.
        self.state_sharpness_attn = nn.Dense(3, name='state_sharpness_attn',
            kernel_init=nn.initializers.zeros,
            bias_init=nn.initializers.zeros)
        self.state_sharpness_rst = nn.Dense(1, name='state_sharpness_rst',
            kernel_init=nn.initializers.zeros,
            bias_init=nn.initializers.zeros)
        self.state_sharpness_range_attn_raw = self.param(
            'state_sharpness_range_attn_raw',
            nn.initializers.zeros, (3,))
        self.state_sharpness_range_rst_raw = self.param(
            'state_sharpness_range_rst_raw',
...
def _attn_forward(x, pool_params, router_params, expand_O_kernel, rng,
    ...
    # v4.1.5.7 state-dependent selection sharpness.  It modulates the
    # match-vs-tau boundary sharpness, not RW execution magnitude.
    state_sharpness_raw_all = (x @ router_params['state_sharpness_attn']['kernel']
                               + router_params['state_sharpness_attn']['bias'])
    state_sharpness_range_attn = _bounded_state_sharpness_range(
        router_params['state_sharpness_range_attn_raw'],
        state_sharpness_range_min, state_sharpness_range_max)  # [3] Q/K/V
    state_sharpness_all = _state_sharpness_from_raw(
        state_sharpness_raw_all, state_sharpness_range_attn)
    attn_state_sharpness_mean = jax.lax.stop_gradient(state_sharpness_all.mean())
    attn_state_sharpness_std = jax.lax.stop_gradient(state_sharpness_all.std())
    attn_state_sharpness_min = jax.lax.stop_gradient(state_sharpness_all.min())
    attn_state_sharpness_max = jax.lax.stop_gradient(state_sharpness_all.max())
    if analysis:
    ...
    state_sharpness_QK = jnp.stack([state_sharpness_all[:, :, 0:1], state_sharpness_all[:, :, 1:2]], axis=2)
    qk_ret = fused_paired(x, h_QK, qk_emb_unit, tau_QK, raw_scan_offset_QK,
                           state_sharpness_QK, qk_read, qk_write)
    ...
    v_ret = fused_single_v(x, h_V, v_emb_unit, tau_all[:, :, 2:3],
                           ...
                           state_sharpness_all[:, :, 2:3], v_read, v_write)
    ...
    slim_ret = (out, aux, qk_active.mean(), v_active.mean(), attn_raw_gmax,
                ...
                attn_state_sharpness_mean, attn_state_sharpness_std,
                attn_state_sharpness_min, attn_state_sharpness_max,
                state_sharpness_range_attn[0],
                state_sharpness_range_attn[1],
                state_sharpness_range_attn[2])
    if not analysis:
        ret = slim_ret
        if return_prune_stats:
            ret = ret + (
...
def _rst_forward(x, pool_params, router_params, rng,
    ...
    state_sharpness_raw = (x @ router_params['state_sharpness_rst']['kernel']
                           + router_params['state_sharpness_rst']['bias'])
    state_sharpness_range_rst = _bounded_state_sharpness_range(
        router_params['state_sharpness_range_rst_raw'],
        state_sharpness_range_min, state_sharpness_range_max)[0]
    state_sharpness = _state_sharpness_from_raw(
        state_sharpness_raw, state_sharpness_range_rst)
    rst_state_sharpness_mean = jax.lax.stop_gradient(state_sharpness.mean())
    rst_state_sharpness_std = jax.lax.stop_gradient(state_sharpness.std())
    rst_state_sharpness_min = jax.lax.stop_gradient(state_sharpness.min())
    rst_state_sharpness_max = jax.lax.stop_gradient(state_sharpness.max())
    if analysis:
    ...
    rst_ret = fused_single(x, h, rst_emb_unit, tau, raw_scan_offset,
                            state_sharpness, rst_read, rst_write)
    ...
    slim_ret = (out, aux, active_frac, raw_gate_max, score_std, gate_sum, active_n_mean,
                ...
                rst_state_sharpness_mean, rst_state_sharpness_std,
                rst_state_sharpness_min, rst_state_sharpness_max,
                state_sharpness_range_rst)
    if not analysis:
        ret = slim_ret
        if return_prune_stats:
            ret = ret + (
...
class DAWN(nn.Module):
    """DAWN-SRW v4.1.5.7 with directional routing and state_sharpness."""
    __version__ = "spatial-r1-v4.1.5.7"
    ...
    # Optional safety bound on router-produced tau offsets.
    tau_offset_clip: Optional[float] = None
    # v4.1.5.7 state_sharpness range: if range is 4, token/state sharpness
    # modulation lives in [1/4, 4] after exp(c * tanh(raw)).
    state_sharpness_range_min: float = 1.25
    state_sharpness_range_max: float = 4.0
    ...
    def setup(self):
        ...
        self.router = Router(
            ...
            state_sharpness_range_min=self.state_sharpness_range_min,
            state_sharpness_range_max=self.state_sharpness_range_max)
        self.layers = [
    ...
    def __call__(self, input_ids, labels=None, attention_mask=None,
        ...
        if self.is_initializing():
            ...
            _ = self.router.state_sharpness_attn(x)
            _ = self.router.state_sharpness_rst(x)
            _ = self.router.state_sharpness_range_attn_raw
            _ = self.router.state_sharpness_range_rst_raw
            ...
            def scan_body(carry, xs):
                ...
                (attn_out, attn_aux, a_qk_active, a_v_active, a_raw_gmax,
                 ...
                 a_state_sharpness_mean, a_state_sharpness_std,
                 a_state_sharpness_min, a_state_sharpness_max,
                 a_state_sharpness_range_q, a_state_sharpness_range_k,
                 a_state_sharpness_range_v) = attn_ret[:40]
                if return_prune_stats:
                    (a_qk_kept_count, a_qk_kept_frac, a_qk_full_gate_sum,
                     ...
                     a_v_int_cap_frac, a_v_gate_max_mean) = attn_ret[40:54]
                if analysis:
                    (a_qk_raw_norm, a_v_raw_norm,
                     ...
                     a_score_kurt, a_int_cap_frac) = attn_ret[40:63]
                if local_diagnostics:
                ...
                (rst_out, rst_aux, k_active, k_raw_gmax, k_sstd, k_gsum,
                 ...
                 k_state_sharpness_mean, k_state_sharpness_std,
                 k_state_sharpness_min, k_state_sharpness_max,
                 k_state_sharpness_range) = rst_ret[:33]
                if return_prune_stats:
                ...
                slim_ys = (attn_aux, rst_aux,
                           ...
                           a_state_sharpness_min, a_state_sharpness_max,
                           a_state_sharpness_range_q, a_state_sharpness_range_k,
                           a_state_sharpness_range_v,
                           k_state_sharpness_mean, k_state_sharpness_std,
                           k_state_sharpness_min, k_state_sharpness_max,
                           k_state_sharpness_range,
                           )
                if return_prune_stats:
                    slim_ys = slim_ys + (
            ...
            attn_state_sharpness_mean_all, attn_state_sharpness_std_all,
            attn_state_sharpness_min_all, attn_state_sharpness_max_all,
            attn_state_sharpness_range_q_all, attn_state_sharpness_range_k_all,
            attn_state_sharpness_range_v_all,
            rst_state_sharpness_mean_all, rst_state_sharpness_std_all,
            rst_state_sharpness_min_all, rst_state_sharpness_max_all,
            rst_state_sharpness_range_all) = scan_ys[:71]
            _scan_offset = 71
            if return_prune_stats:
        ...
        result = {
            ...
            'attn_state_sharpness_mean': attn_state_sharpness_mean_all.mean(),
            'attn_state_sharpness_std': attn_state_sharpness_std_all.mean(),
            'attn_state_sharpness_min': attn_state_sharpness_min_all.min(),
            'attn_state_sharpness_max': attn_state_sharpness_max_all.max(),
            'attn_state_sharpness_range_q': attn_state_sharpness_range_q_all.mean(),
            'attn_state_sharpness_range_k': attn_state_sharpness_range_k_all.mean(),
            'attn_state_sharpness_range_v': attn_state_sharpness_range_v_all.mean(),
            'rst_state_sharpness_mean': rst_state_sharpness_mean_all.mean(),
            'rst_state_sharpness_std': rst_state_sharpness_std_all.mean(),
            'rst_state_sharpness_min': rst_state_sharpness_min_all.min(),
            'rst_state_sharpness_max': rst_state_sharpness_max_all.max(),
            'rst_state_sharpness_range': rst_state_sharpness_range_all.mean(),
        }
