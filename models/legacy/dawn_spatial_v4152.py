"""
DAWN-SRW v4.1.5.2: Sense-Read-Write routing.

This file contains the active v4.1.5.2 implementation.  Routing is performed
in the learned route space:

    score_i = h @ emb_i

The read and write vectors define the SRW operator applied after routing:
read selects a scalar feature from the token state, and write maps that scalar
back into model space.

Core objects
------------
NeuronPool:
    qk_emb, v_emb, know_emb         [N, d_route]   routing embeddings
    qk_read, v_read, know_read      [N, d_model]   input directions
    qk_write, v_write, know_write   [N, d_model]   output directions
    qk_scale, v_scale, know_scale   scalar output scales

Router:
    proj_attn: x -> h_Q, h_K, h_V in d_route space
    proj_know: x -> h_know in d_route space
    tau_*: learned relative threshold offsets
    scan_bias_*: bounded recruitment axis
...
* The denominator is activation-weighted intensity: sum(gate).
...
# ================================================================
# 5. Router -- route queries, tau offsets, bounded scan bias
# ================================================================
...
class DAWN(nn.Module):
    ...
    def __call__(self, input_ids, labels=None, attention_mask=None,
        ...
        if self.is_initializing():
            ...
            _ = self.neuron_pool.qk_emb  # triggers NeuronPool.setup
