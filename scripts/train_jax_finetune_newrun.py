...
def create_train_step(model, optimizer, orth_weight, div_weight, lb_weight,
    ...
    def train_step(params, opt_state, input_ids, attention_mask, dropout_key,
        ...
        # Emb drift is DAWN/SRW-only. Baseline Transformer has no neuron_pool.
        # Keep these diagnostic slots present as zeros so the shared logger and
        # train_step signature stay identical without touching baseline params.
        if is_baseline:
            drift_attn_qk_emb = jnp.float32(0.0)
            drift_attn_v_emb = jnp.float32(0.0)
            drift_rst_emb = jnp.float32(0.0)
        else:
            # Computed inside jit so every host participates in the norm collectives.
...
def load_checkpoint_params_only(path, target_params):
    """Load only params from a checkpoint. Optimizer/step are intentionally ignored.

    This is for fine-tuning/new-run initialization from a pretraining checkpoint.
    It preserves the original training graph and optimizer construction while
    avoiding opt_state structure mismatches across experiments.
    """
    import flax.serialization as serialization
    with _open_file(path, 'rb') as f:
        bytes_data = f.read()
    ...
    if jax.process_index() == 0:
        print(f"  Checkpoint loaded (params only): {path}")
    return {
        'params': params,
        'epoch': 0,
        'step': 0,
...
def main():
    ...
    parser.add_argument('--init-from', type=str, default=None,
                        help='Initialize params from a checkpoint file or run folder, then create a fresh new run with fresh optimizer')
    cli_args = parser.parse_args()
    if cli_args.init_from and cli_args.resume_from:
        parser.error('--init-from and --resume-from are mutually exclusive. Use --init-from to start a new fine-tune run; use --resume-from to resume that new run later.')
    ...
    def _resolve_init_checkpoint(path):
        """Resolve --init-from path. If a run folder is given, prefer best_model.flax."""
        p = path.rstrip('/')
        if p.endswith('.flax'):
            if _file_exists(p):
                return p
            return None
        best = _join(p, 'best_model.flax')
        if _file_exists(best):
            return best
        step_candidates = _list_files(p, 'checkpoint_step*.flax')
        if step_candidates:
            return step_candidates[-1]
        any_candidates = _list_files(p, '*.flax')
        if any_candidates:
            return any_candidates[-1]
        return None
    ...
    if cli_args.init_from:
        _host0_init_path = None
        _host0_explicit_missing = False
        if jax.process_index() == 0:
            _host0_init_path = _resolve_init_checkpoint(cli_args.init_from)
            if _host0_init_path:
                print(f"  Initializing new run from: {_host0_init_path}")
            else:
                _host0_explicit_missing = True
                print(f"  No .flax checkpoint found for --init-from {cli_args.init_from}")
        init_path = _broadcast_str_from_host0(_host0_init_path)
        _missing_signal = _broadcast_str_from_host0(
            'MISSING' if _host0_explicit_missing else '')
        if _missing_signal == 'MISSING':
            raise FileNotFoundError(
                f"No .flax checkpoint found for --init-from {cli_args.init_from}")
    ...
    if init_path and _file_exists(init_path):
        if is_host0:
            print(f"\nInitializing new run from: {init_path}")
        ckpt = load_checkpoint_params_only(init_path, params)
        params = ckpt['params']
        opt_state = optimizer.init(params)
        start_epoch = 0
        global_step = 0
        ...
        best_val_loss = float('inf')
        if is_host0:
            print("  Init-newrun: params loaded, optimizer=fresh, step=0")
            print(f"  New checkpoints/logs will be written only to: {checkpoint_dir}")
    elif resume_path and _file_exists(resume_path):
        if is_host0:
        ...
        if is_host0:
            if not cli_args.from_scratch and not cli_args.init_from:
                print("\nNo checkpoint found. Starting from scratch.")
            elif cli_args.from_scratch:
    ...
    try:
        ...
        def _drift_snap(p):
            # Baseline Transformer has no neuron_pool; return tiny placeholders.
            # create_train_step ignores them when is_baseline=True.
